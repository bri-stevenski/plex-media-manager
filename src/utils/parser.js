...
exports.parseMediaFile = parseMediaFile;
...
function parseTvFilename(filename) {
    const match = constants_1.SEASON_EPISODE_REGEX.exec(filename);
    if (match) {
        if (match[1] && match[2]) {
            // 6X10 pattern
 ...
 */
function parseDateInFilename(filename) {
    for (const rx of constants_1.DATE_REGEXES) {
 ...
 */
function parseSeasonFromPath(filepath) {
 ...
 */
function inferShowDirectoryFromPath(filepath) {
    const parent = path_1.default.basename(path_1.default.dirname(filepath));
    if (/^Season\s+\d{1,2}$/i.test(parent) || /^Specials$/i.test(parent)) {
        return path_1.default.basename(path_1.default.dirname(path_1.default.dirname(filepath)));
    }
    return parent;
}
/**
 ...
 */
function parseMovieDirectoryFromPath(filepath) {
 ...
 */
function extractYearFromStem(stem) {
    const matches = stem.matchAll(new RegExp(constants_1.YEAR_REGEX.source, 'g'));
    let lastYear = null;
 ...
 */
function guessTitleAndYearFromStem(stem) {
    ...
    if (parenthesesMatch) {
        ...
        if (year) {
            const yearMatch = Array.from(s.matchAll(new RegExp(constants_1.YEAR_REGEX.source, 'g'))).find((m) => parseInt(m[0]) === year);
            if (yearMatch) {
    ...
    titlePart = titlePart.replace(constants_1.SEASON_EPISODE_REGEX, '');
    // Remove quality/format tags
    titlePart = titlePart.replace(constants_1.QUALITY_FORMATS_REGEX, '');
 ...
 */
function extractEpisodeTitleFromFilename(stem) {
    ...
    for (const pattern of patternsOriginal) {
        ...
        if (match) {
            ...
            if (titleCandidate &&
                ...
                !constants_1.SEASON_EPISODE_REGEX.test(titleCandidate)) {
    ...
    for (const pattern of patternsNormalized) {
        ...
        if (match) {
            ...
            if (titleCandidate &&
                !constants_1.SEASON_EPISODE_REGEX.test(titleCandidate) &&
                !constants_1.QUALITY_FORMATS_REGEX.test(titleCandidate)) {
                return sanitizeFilename(titleCandidate);
            }
        }
    }
...
function parseMediaFile(filepath) {
    const stem = path_1.default.basename(filepath, path_1.default.extname(filepath));
    ...
    if ((isSeasonBasedTv || isDateBasedTv) && !isInMoviesFolder) {
        // TV Show
        const pathParts = path_1.default.normalize(filepath).split(path_1.default.sep);
        let showDir = null;
        ...
        return {
            content_type: constants_1.CONTENT_TYPE_TV,
            title: showTitle,
            year,
            season,
            episode,
        ...
        if (movieDirInfo) {
            ...
            const stemContainsFolderTitle = normalizedDirTitle.length >= 4 && normalizedStemTitle.includes(normalizedDirTitle);
        ...
        return {
            content_type: constants_1.CONTENT_TYPE_MOVIES,
            title,
            year,
            season: null,
            episode: null,
